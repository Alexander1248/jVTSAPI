...
public enum ErrorID
{
    ...
    Event_TestEvent_TestMessageTooLong(EVENT_OFFSET.getCode()),

    // Event subscription errors for ModelLoadedEvent
    Event_ModelLoadedEvent_ModelIDInvalid(EVENT_OFFSET.getCode() + 50);
    ...
    public static ErrorID byCode(int code) {
        for (ErrorID m : ErrorID.values()) {
            if (m.code == code) {
                return m;
            }
        }
